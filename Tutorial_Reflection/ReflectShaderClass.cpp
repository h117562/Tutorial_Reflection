...
ReflectShaderClass::ReflectShaderClass(const ReflectShaderClass& other)
{
}

bool ReflectShaderClass::Initialize(ID3D11Device* pDevice, HWND hwnd)
{
	HRESULT result;
	...
	const wchar_t* vsFilename = L"../ReflectVS.hlsl";
	const wchar_t* psFilename = L"../ReflectPS.hlsl";
	...
	//매트릭스 버퍼 설정
	matrixBufferDesc.ByteWidth = sizeof(ExtendedMatrixBuffer);//버퍼 크기
...
//에러 내용을 메시지 박스로 출력
void ReflectShaderClass::OutputShaderErrorMessage(ID3DBlob* errorMessage, HWND hwnd)
{
	char* compileErrors;
	...
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, XMMATRIX reflectionMatrix)
{
	...
	ExtendedMatrixBuffer* dataPtr = 0;
	...
	dataPtr = (ExtendedMatrixBuffer*)mappedResource.pData;
